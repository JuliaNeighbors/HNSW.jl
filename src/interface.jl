...
function HierarchicalNSW(vector_type::Type;
    ...
    return HierarchicalNSW{T,F,Vector{vector_type},typeof(metric)}(
        lg, Bool[], vector_type[], ep, 0, Mutex(), vlp, metric, efConstruction, ef)
end
...
function add_to_graph!(notify_func::Function,
    hnsw::HierarchicalNSW{T}, indices; multithreading=false) where {T}
    ...
    if multithreading
        ...
        Threads.@threads for i ∈ indices
            if !hnsw.added[i]
                insert_point!(hnsw, T(i), levels[i])
                hnsw.added[i] = true
                notify_func(i)
            end
        end
...
short form of `add_to_graph!(notify_func, hnsw, indices; multithreading=multithreading)`
"""
add_to_graph!(hnsw::HierarchicalNSW{T}, indices; kwargs...) where {T} =
    add_to_graph!(identity, hnsw, indices; kwargs...)
